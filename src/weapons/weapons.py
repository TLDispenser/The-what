...
class Weapon:
    ...
    def shoot(self, x, y, direction, source, bullets):
        pass
    ...
    def shoot(self, x, y, direction, source, bullets):
        if not self.did_fire:
            if self.ammo > 0 and source == "Player":
                self.ammo -= 1
                bullets.append(fireables.Bullet(x, y, direction, source))
                self.did_fire = True
        self.did_fire = True
...
class RocketLauncher(Weapon):
    ...
    def shoot(self, x, y, direction, source, bullets):
        if not self.did_fire:
            if self.ammo > 0 and source == "Player":
                self.ammo -= 1
                # Add rocket shooting logic here
                bullets.append(fireables.Bullet(x, y, direction, source))  # Placeholder for rocket
                self.did_fire = True
        self.did_fire = True
...
class RotaryGun(Weapon):
    ...
    def special_shoot(self, x, y, direction, source, bullets):
        if self.did_fire:
            if self.refire_timer > self.refire_time * 60:
                # Add special bullet shooting logic here
                self.ammo -= 1
                bullets.append(fireables.Bullet(x, y, direction, source))
                self.refire_timer = 0
            else:
